...
const Settings: React.FC = () => {
  ...
  /* Notifications (UI-only — extend if your backend supports it) */
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Backend may return full name or split first/last
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      // Update cached user
      const cached = JSON.parse(localStorage.getItem("cb_user") ?? "{}");
  ...
  const savePassword = async () => {
    ...
    setLoadingPw(true);
    try {
      const res = await fetch(`${API}/api/user/password/change`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({
  ...
  const NOTIF_ITEMS: {
  ...
  }[] = [
    {
      ...
      desc: "Get notified when a deposit is confirmed on-chain.",
    },
    {
    ...
    },
    {
      key: "kycStatus",
      label: "KYC Status",
      desc: "Updates on your identity verification process.",
    },
    {
      key: "securityAlerts",
      label: "Security Alerts",
      desc: "Login attempts and account security events.",
    },
    {
      key: "marketing",
      label: "Product Updates",
      desc: "New features, promotions and announcements.",
    },
    {
      key: "weeklyReport",
    ...
    <div className="st page-enter">
        ...
        <div className="st-card">
          <div className="st-avatar-row">
            <div className="st-avatar">
              <span>{initials}</span>
              <button className="st-avatar-edit" aria-label="Change photo">
                <IcoCamera />
              </button>
            </div>
            <div>
      ...
      {tab === "security" && (
        <div className="st-card">
          ...
          <div className="st-form">
            <div className="input-wrap">
              ...
              <div className="st-pw-wrap">
                <input
                  className="input-field st-pw-input"
                ...
                <div className="st-pw-strength">
                  <div className="st-pw-bars">
                    {[1, 2, 3, 4].map((n) => (
                      <div
                        key={n}
                        className={`st-pw-bar ${strength >= n ? strengthCls : ""}`}
                      />
                    ))}
                  </div>
                  <span className={`st-pw-label ${strengthCls}`}>
                    {strengthLabel}
                  </span>
                </div>
              )}
          ...
          <div className="st-card-footer">
            <button
              className={`wd-btn-primary ${pwSaved ? "st-saved-btn" : ""}`}
